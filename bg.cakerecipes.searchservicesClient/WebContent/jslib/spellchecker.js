...
exports.train = speller.train = function(text) {
	var word, m, r = /[А-Яа-я]+/g;
	text = (text != undefined) ? text.toLowerCase() : "";
	while ((m = r.exec(text))) {
		word = m[0];
		speller.nWords[word] = speller.nWords.hasOwnProperty(word) ? speller.nWords[word] + 1
				: 1;
	}
};
...
exports.correct = speller.correct = function(word) {
	...
	list.forEach(function(edit) {
		speller.edits(edit).forEach(function(w) {
			if (speller.nWords.hasOwnProperty(w))
				candidates[speller.nWords[w]] = w;
		});
	});
...
// A helper function that returns the word with the most occurences in the
// language
...
speller.letters = "абвгдежзийклмнопрстуфхцчшщъьюя".split("");
